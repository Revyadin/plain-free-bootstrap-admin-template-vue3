...
const fetchProducts = async () => {
  try {
    const res = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/products?q=${searchTerm.value}&limit=10&typeTransactions=${mode.value}`)
...
const handleCheckout = async ({ tunai, nontunai, total, kembalian }) => {
  ...
  try {
    ...
    const items = detailcart.value.map(item => ({
